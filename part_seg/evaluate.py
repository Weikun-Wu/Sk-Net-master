...
parser.add_argument('--model', default='part_seg', help='Model name [default: part_seg]')
...
parser.add_argument('--num_keypoints', type=int, default=128, help='Number of keypoints [default: 128]')
parser.add_argument('--k', type=int, default=16, help='Number of k-nearest neighbors for spatial pattern [default: 16]')
parser.add_argument('--nsample', type=int, default=64, help='Number of k-nearest neighbors for model details [default: 64]')
...
DATA_PATH =  '/home/wuweikun/xxx/data/shapenetcore_partanno_segmentation_benchmark_v0_normal'
...
def evaluate():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl,cls_label = cls_labels_pl,
                                              NUM_CLASSES = NUM_CLASSES, NUM_CATEGORIES = NUM_CATEGORIES,
...
def eval_one_epoch(sess, ops):
    ...
    seg_classes = TEST_DATASET.seg_classes#{'Earphone': [16, 17, 18], 'Motorbike': [30, 31, 32, 33, 34, 35],...}
    shape_ious = {cat:[] for cat in seg_classes.keys()}#16 cats:{'Earphone:[],...}
    seg_label_to_cat = {} # {0:Airplane, 1:Airplane, ...49:Table}
    for cat in seg_classes.keys():#16
        for label in seg_classes[cat]:#[16, 17, 18] for Earphone
    ...
    for batch_idx in range(num_batches):
        ...
        # Select valid data
        cur_pred_val = pred_val[0:cur_batch_size]#BxNxClass
        # Constrain pred to the groundtruth classes (selected by seg_classes[cat]:[...])
        cur_pred_val_logits = cur_pred_val
        cur_pred_val = np.zeros((cur_batch_size, NUM_POINT)).astype(np.int32)
        for i in range(cur_batch_size):
            cat = seg_label_to_cat[cur_batch_label[i,0]]# cat for every point
            logits = cur_pred_val_logits[i,:,:]
            cur_pred_val[i,:] = np.argmax(logits[:,seg_classes[cat]], 1) + seg_classes[cat][0] #max pred
