...
class PartNormalDataset():
    def __init__(self, root, npoints = 2500, classification = False, split='train', normalize=True, return_cls_label = False):
        ...
        with open(self.catfile, 'r') as f:
            for line in f:
                ls = line.strip().split()
                self.cat[ls[0]] = ls[1]#e.g. cat['Bag'] = 02773838
        self.cat = {k:v for k,v in self.cat.items()}#dict
        #print(self.cat)
        ...
        for item in self.cat: # keys
            ...
            fns = sorted(os.listdir(dir_point))#listdir:return the files in dir_point;and sort them
            #print(fns[0][0:-4]),ignore str '.txt'
            if split=='trainval':
            ...
            #print(os.path.basename(fns)),return the last file-name of fns
            #os.path.splitext,split the file-name from expanded-name
            for fn in fns:
                token = (os.path.splitext(os.path.basename(fn))[0]) 
                self.meta[item].append(os.path.join(dir_point, token + '.txt')) #e.g./02773838/s324ssdas...ad2.txt
        
        self.datapath = []
        for item in self.cat:#keys
            for fn in self.meta[item]:
                self.datapath.append((item, fn))#e.g.('Bag','/02773838/s324ssdas...ad2.txt')
            
         
        self.classes = dict(zip(self.cat, range(len(self.cat))))#{'Bag':2,...}
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            cat = self.datapath[index][0]#e.g.'Bag'
            cls = self.classes[cat]#label,'Bag':2,so get 2
