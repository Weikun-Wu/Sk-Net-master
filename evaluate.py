...
parser.add_argument('--num_point', type=int, default=5000, help='Point Number [default: 1024]')
parser.add_argument('--model_path', default='ModelNet10_96.2%/model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
parser.add_argument('--num_keypoints', type=int, default=192, help='Number of keypoints [default: 128]')
parser.add_argument('--k', type=int, default=16, help='Number of k-nearest neighbors for spatial pattern [default: 16]')
parser.add_argument('--nsample', type=int, default=128, help='Number of k-nearest neighbors for model details [default: 32]')
...
if FLAGS.modelnet10:
    SHAPE_NAMES = [line.rstrip() for line in \
        open(os.path.join(DATA_PATH, 'modelnet10_shape_names.txt'))]
    NUM_CLASSES = 10

else:
    SHAPE_NAMES = [line.rstrip() for line in \
        open(os.path.join(DATA_PATH,'modelnet40_shape_names.txt'))]
    NUM_CLASSES = 40
...
def evaluate(num_votes):
    ...
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        # simple model
        pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, normal = FLAGS.normal,
