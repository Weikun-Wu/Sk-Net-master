...
__global__ void gatherdimspointKernel(int b,int n,int m,int c,const float * __restrict__ inp,const int * __restrict__ idx,float * __restrict__ out){
  int batch_index = blockIdx.x;
  inp += n*c*batch_index;
  idx += m*batch_index;
  out += m*c*batch_index;
 
  int channels = threadIdx.x;
  for(int i=0;i<m;i++){
    	int j = idx[i];
    	out[i*c+channels] = inp[j*c+channels];
  }
}
...
void gatherdimspointLauncher(int b,int n,int m,int c ,const float * inp,const int * idx,float * out){
  gatherdimspointKernel<<<b,c>>>(b,n,m,c,inp,idx,out);
}
