...
void gatherdimspointLauncher(int b,int n,int m,int c,const float * inp,const int * idx,float * out);
class GatherDimsPointGpuOp: public OpKernel{
  public:
    explicit GatherDimsPointGpuOp(OpKernelConstruction * context):OpKernel(context){}
    void Compute(OpKernelContext * context)override{
      const Tensor& inp_tensor=context->input(0);
      int b=inp_tensor.shape().dim_size(0);
      int n=inp_tensor.shape().dim_size(1);
      int c = inp_tensor.shape().dim_size(2);
      OP_REQUIRES(context,inp_tensor.dims()==3 && inp_tensor.shape().dim_size(2)==c,errors::InvalidArgument("GatherDimsPoint expects (batch_size,num_points,c) inp shape"));
      
      const Tensor& idx_tensor=context->input(1);
      OP_REQUIRES(context,idx_tensor.dims()==2 && idx_tensor.shape().dim_size(0)==b,errors::InvalidArgument("GatherDimsPoint expects (batch_size,num_result) idx shape"));
      int m=idx_tensor.shape().dim_size(1);
      auto inp_flat=inp_tensor.flat<float>();
      const float * inp=&(inp_flat(0));
      auto idx_flat=idx_tensor.flat<int>();
      ...
      auto out_flat=out_tensor->flat<float>();
      float * out=&(out_flat(0));
      gatherdimspointLauncher(b,n,m,c,inp,idx,out);
    }
};
REGISTER_KERNEL_BUILDER(Name("GatherDimsPoint").Device(DEVICE_GPU),GatherDimsPointGpuOp);
