...
def PReLU(x,name='prelu'):
  gamma = tf.Variable(tf.constant(0.25),dtype=tf.float32)
  return tf.maximum(x,gamma*x,name='prelu')
...
def upConv2d(inputs,
  ...
  with tf.variable_scope(scope) as sc:
    ...
    upsample_outputs = tf.layers.conv2d_transpose(inputs,
    ...
    biases = _variable_on_cpu('biases', [num_output_channels],
                                tf.constant_initializer(0.0))
    outputs = tf.nn.bias_add(upsample_outputs, biases, data_format=data_format)
    if bn:
