...
def showpoints(xyz,c_gt=None, c_pred = None ,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(0,0,0),normalizecolor=True,ballradius=5):
    ...
    def render():
        ...
        if showrot:
            ...
            cv2.putText(show,'xangle %d'%(int(xangle/np.pi*180)),(30,showsz-30),0,0.5,(100,100,100))
            cv2.putText(show,'yangle %d'%(int(yangle/np.pi*180)),(30,showsz-50),0,0.5,(255,255,255))
        ...
        if cmd==ord('t') or cmd == ord('p'):
            if cmd == ord('t'):
                if c_gt is None:
                    ...
                    c1=np.zeros((len(xyz),),dtype='float32')+250
...
if __name__=='__main__':
    np.random.seed(100)
    c_pred = np.zeros((2500,3),dtype='float32')
    print(c_pred.shape)
    ...
    c_pred[2000:2500,0] = 0
    c_pred[2000:2500,1] = 255
    c_pred[2000:2500,2] = 0
    showpoints(np.random.randn(2500,3),c_pred = c_pred)
